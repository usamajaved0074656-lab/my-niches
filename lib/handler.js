...
async function fetchHtml(url) {
  ...
  if (r.status === 404) throw new Error('YouTube returned 404 for that link — check the handle.');
...
async function resolveYouTube(rawUrl) {
  ...
  if (!out.title) throw new Error('Could not read channel info from that link.');
  return out;
}
