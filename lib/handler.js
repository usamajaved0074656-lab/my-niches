...
import {
  ...
  noteFailure, noteSuccess, passwordSet, sessionCookie,
} from '../auth.js';
...
async function buildChannel(meta, extra = {}) {
  ...
  return {
    ...
    channelId: /\/channel\/(UC[\w-]{20,})/.exec(meta.canonical || meta.channelUrl || '')?.[1] || '',
...
/* ---------- removed channel sweep ---------- */

/**
 * Probes whether a channel is still on YouTube.
 * Returns { state: 'alive', channelId }, { state: 'gone', reason }, or { state: 'unknown' }.
 * Never throws — a network error or rate limit is 'unknown', never 'gone'.
 */
export async function probeChannel(ch) {
  try {
    if (!ch) return { state: 'unknown' };
    const id = ch.channelId || /\/channel\/(UC[\w-]{20,})/.exec(ch.url || '')?.[1];
    const target = id
      ? `https://www.youtube.com/channel/${id}`
      : (ch.handle
          ? (ch.handle.startsWith('http') ? ch.handle : `https://www.youtube.com/${ch.handle.startsWith('@') ? ch.handle : '@' + ch.handle}`)
          : ch.url);
    if (!target) return { state: 'unknown' };

    try {
      const html = await fetchHtml(target);
      const hasOgTitle = Boolean(pick(html, /<meta property="og:title" content="([^"]+)"/)) || /<meta[^>]+property=["']og:title["']/i.test(html);
      if (hasOgTitle) {
        const canonical = pick(html, /<link rel="canonical" href="([^"]+)"/);
        const canonicalId = /\/channel\/(UC[\w-]{20,})/.exec(canonical || '')?.[1];
        return { state: 'alive', channelId: canonicalId || id || '' };
      }
      const reason = goneReason(decodeEntities(html.replace(/<[^>]+>/g, ' ')));
      if (reason) return { state: 'gone', reason };
    } catch (e) {
      if (e instanceof ChannelGone) {
        return { state: 'gone', reason: e.reason };
      }
      // Any other error falls through to yt-dlp
    }

    const alt = await ytdlpChannel(target, 1, 1);
    if (alt?.gone) return { state: 'gone', reason: alt.gone };
    if (alt?.title) return { state: 'alive', channelId: id || '' };
...
/**
 * Daily sweep of all saved channels to check if any have been taken down by YouTube.
 * Processes sequentially with gapMs delay between probes to avoid triggering rate limits.
 * Never throws — individual job errors are logged and the sweep continues.
 */
export async function sweepRemoved({ gapMs = 15000, only = null, log = console.log } = {}) {
  let checked = 0;
  ...
  try {
    const niches = (await store.all()) || [];
    const jobs = [];

    const matchesOnly = (c) => {
      if (!only) return true;
      if (typeof only.has === 'function') {
        return only.has(c.id) || Boolean(c.channelId && only.has(c.channelId));
      }
      if (Array.isArray(only)) {
        return only.includes(c.id) || Boolean(c.channelId && only.includes(c.channelId));
      }
      return true;
    };
    ...
    for (const n of niches) {
      for (const c of n.channels || []) {
        if (c && c.id && matchesOnly(c)) {
          jobs.push({ nicheId: n.id, channelId: c.id });
        }
      }
    }

    for (let j = 0; j < jobs.length; j++) {
      const { nicheId, channelId } = jobs[j];
      try {
        const niche = await store.get(nicheId);
        const channel = (niche?.channels || []).find((c) => c.id === channelId);
        if (!channel) continue;

        checked++;
        const r = await probeChannel(channel);
        if (r.state === 'unknown') {
          unknown++;
        } else {
          // A probe takes seconds and the owner may have edited this niche meanwhile,
          // so re-read it now and change only this one channel on the fresh copy.
          const n = await store.get(nicheId);
          ...
          if (i !== -1) {
            ...
            if (r.state === 'gone' && !c.removed) {
              next = { ...c, removed: true, removedAt: new Date().toISOString(), removedReason: r.reason };
              movedToRemoved++;
            } else if (r.state === 'alive' && c.removed) {
              const { removed, removedAt, removedReason, ...rest } = c;
              next = rest;
              restored++;
            }

            if (r.state === 'alive' && r.channelId && !c.channelId) {
              next = { ...(next || c), channelId: r.channelId };
            }

            if (next) {
              channels[i] = next;
              await store.update(nicheId, { channels });
            }
          }
        }
      } catch (err) {
        log(`removed-sweep: error on channel ${channelId}:`, err);
      }

      if (j < jobs.length - 1 && gapMs > 0) {
        await new Promise((resolve) => setTimeout(resolve, gapMs));
      }
    }
  } catch (err) {
    log('removed-sweep: fatal error:', err);
  }

  log(`removed-sweep: checked ${checked} · moved ${movedToRemoved} to Removed · ${restored} restored · ${unknown} unknown`);
  return { checked, movedToRemoved, restored, unknown };
}
...
export async function handleRequest(req, res) {
  try {
    ...
    if (p === '/api/logout' && req.method === 'POST') {
      // Clears this browser only. Signing every device out is a password
      // change (the password is the HMAC key), not one person's logout.
