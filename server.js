 ...
 */
const HOSTED = Boolean(process.env.RENDER || process.env.FLY_APP_NAME || process.env.RAILWAY_ENVIRONMENT);
const die = (msg) => {
  console.error(`\n  ${msg}\n`);
  process.exit(1);
};
...
if (HOSTED && !process.env.APP_PASSWORD) {
  die('APP_PASSWORD set nahi hai. Public deploy bina password ke nahi chalega — Render ke Environment mein APP_PASSWORD daalo.');
}
if (process.env.APP_PASSWORD && process.env.APP_PASSWORD.length < 8) {
  die('APP_PASSWORD bohat chhota hai — kam az kam 8 characters (16+ behtar).');
}
...
if (Boolean(process.env.SUPABASE_URL) !== Boolean(process.env.SUPABASE_KEY)) {
  die('SUPABASE_URL aur SUPABASE_KEY — dono chahiye ya dono khaali. Abhi sirf ek set hai.');
}
if (HOSTED && !process.env.SUPABASE_URL) {
  die('Hosted par Supabase lazmi hai — warna data har restart par mit jayega. SUPABASE_URL aur SUPABASE_KEY set karo.');
}

const store = createStore({ dbPath: DB });
 ...
 */
const MIRROR =
  process.env.MIRROR_IMAGES === '0' ? false : !(process.env.RENDER || process.env.MIRROR_IMAGES === '0');
